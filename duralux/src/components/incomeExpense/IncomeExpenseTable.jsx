...
export default function IncomeExpenseTable({ filters }) {
  ...
  const buildActiveFilter = (activeFilter) => {
    if (activeFilter === "INCOME") return { type: "INCOME" };
    if (activeFilter === "EXPENSE") return { type: "EXPENSE" };
    if (activeFilter === "PENDING") return { isPaid: "false" };
    if (activeFilter === "COLLECTED") return { isPaid: "true" };
    return {};
  };
  ...
  useEffect(() => {
    ...
    const params = {
      ...baseParams,
      ...buildActiveFilter(activeFilter),
  ...
  }, [filters, searchTerm, activeFilter]);
  ...
  const getRemainingDays = (dueDate) => {
    ...
    if (dueDate === null || diff === 999999)
      return { text: "—", class: "text-muted opacity-50" };
    if (diff < 0)
      return {
            ...
            >
              ...
              >
                <div className="card-body">
                  ...
                  <h4 className="fw-bold mb-0" style={{ color: card.text }}>
              ...
              <tr className="small fw-bold text-uppercase text-muted">
                ...
                <th className="py-3 ps-4" style={{ minWidth: "320px" }}>
                  <div className="d-flex align-items-center gap-3">
                    <div
                      className="pointer d-flex align-items-center flex-shrink-0"
                ...
                <th
                  className="text-danger opacity-75 pointer"
                ...
                <th
                  ...
                  onClick={() => requestSort("remainingDays")}
                >
                  Gün{" "}
              ...
              {processedItems.length > 0 ? (
                processedItems.map((x) => {
                  const remainingAmount =
                    Number(x.amount) - (Number(x.paidAmount) || 0);
                  ...
                  return (
                    <tr
                      key={x.id}
                      style={{
                        backgroundColor: isGecikti ? "#fffcfc" : "transparent",
                      }}
                    >
                      <td className="ps-4">
                        {/* Müşteri İsmi */}
                      ...
                      <td>
                        <span
                          className={`badge rounded-pill fw-medium ${x.type === TransactionType.INCOME ? "bg-success-subtle text-success" : "bg-danger-subtle text-danger"}`}
                      ...
                      <td className="text-muted" style={{ fontSize: "12px" }}>
                        {x.dueDate
                          ? new Date(x.dueDate).toLocaleDateString("tr-TR")
                          : "—"}
                      </td>
                      <td className={`small fw-medium ${dayInfo.class}`}>
                        {dayInfo.text}
                      </td>
                      <td>
                        <span
                          className={`badge ${remainingAmount <= 0 ? "bg-success" : isGecikti ? "bg-danger" : "bg-warning text-dark"}`}
                          style={{ fontSize: "10px" }}
                        >
                          {remainingAmount <= 0
                            ? "Ödendi"
                      ...
                      <td className="text-end pe-4">
                        <div className="d-flex justify-content-end gap-1">
                          <button
                            className="btn btn-sm btn-light avatar-sm"
                            onClick={() =>
                          ...
                          <button
                            className="btn btn-sm btn-light avatar-sm text-primary"
                            onClick={() =>
