...
import { FiTrash2, FiEdit2, FiSave, FiX, FiCalendar, FiUser, FiBriefcase, FiCheckCircle } from 'react-icons/fi'
        ...
        <div className="modal-dialog modal-lg modal-dialog-centered">
          <div className="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            ...
            <div className="modal-header border-0 pb-0 pt-4 px-4 align-items-start">
              <div>
                <span className={`badge rounded-pill bg-${ui.color} bg-opacity-10 text-${ui.color} px-3 py-2 mb-2 border border-${ui.color} border-opacity-25`}>
                  {ui.label}
                </span>
            ...
            <div className="modal-body p-4">
              ...
              <div className={`rounded-3 p-4 mb-4 border transition-all ${editMode ? 'bg-white border-primary shadow-sm' : 'bg-light bg-opacity-50 border-light'}`}>
                
                {/* BAŞLIK */}
                <div className="mb-3">
                  <small className="text-uppercase text-muted fw-bold d-block mb-1" style={{fontSize: '0.75rem'}}>Başlık</small>
                  {editMode ? (
                    <input
                ...
                {/* AÇIKLAMA */}
                <div>
                  <small className="text-uppercase text-muted fw-bold d-block mb-1" style={{fontSize: '0.75rem'}}>Açıklama</small>
                  {editMode ? (
                    <textarea
                    ...
                    <p className="text-secondary mb-0" style={{ whiteSpace: 'pre-wrap', lineHeight: '1.6' }}>
                      {activity.description || <span className="text-muted fst-italic">Açıklama girilmemiş.</span>}
                    </p>
                  )}
                </div>
                  ...
                  <div className="col-md-6">
                    <div className="card h-100 border-0 shadow-sm bg-white">
                      <div className="card-body">
                        ...
                        <small className="text-uppercase text-muted fw-bold d-block mb-1" style={{ fontSize: '0.75rem' }}>Müşteri</small>
                        <div className="fw-bold text-dark">
                          <Link href={`/customers/view/${activity.customer.id}`} className="text-decoration-none text-dark stretched-link">
                  ...
                  <div className="col-md-6">
                     <div className="card h-100 border-0 shadow-sm bg-white">
                      <div className="card-body">
                        ...
                        <small className="text-uppercase text-muted fw-bold d-block mb-1" style={{ fontSize: '0.75rem' }}>Bağlı Görev</small>
                        <div className="fw-bold text-dark">
                          <Link href={`/tasks/view/${activity.task.id}`} className="text-decoration-none text-dark stretched-link">
                            {activity.task.title}
                          </Link>
                        </div>
                      </div>
              ...
              {!editMode && activity.createdByUser && (
                 <div className="d-flex align-items-center justify-content-end text-muted small">
                    ...
                    <span className="fw-bold text-dark">{activity.createdByUser.username}</span>
                 </div>
              )}
